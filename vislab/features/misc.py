...
def size(image_ids, image_filenames):
    ...
    """
    print "number of input images %d" % len(image_ids)
    good_image_ids = []
    feats = []
    bad_cnt = 0
    for image_id, filename in zip(image_ids, image_filenames):
        try:
            image = vislab.image.get_image_for_filename(filename)
            ...
            bad_cnt += 1
            continue
    #print len(image_ids), len(good_image_ids), bad_cnt
    return good_image_ids, feats
...
def gist(image_ids, image_filenames, max_size=256):
    ...
    for image_id, filename in zip(image_ids, image_filenames):
        try:
            ...
            img = vislab.image.get_image_for_filename(filename)
...
def gbvs_saliency(image_ids, image_filenames):
    ...
    matlab_cmd = "matlab -nojvm -r \"try; {}; catch; exit; end; exit\"".format(
        matlab_script)
    print(type(matlab_cmd))

    pid = subprocess.Popen(
        shlex.split(matlab_cmd),
        cwd='/data/qzhang/work/vislab-git/matlab/gbvs')
    retcode = pid.wait()
    if retcode != 0:
        raise Exception("Matlab script did not exit successfully!")
    print "hello world\n\n"
...
def mc_bit(image_ids, image_filenames):
    ...
    print len(glob.glob(output_dirname + '/*_mc_bit.ascii'))
